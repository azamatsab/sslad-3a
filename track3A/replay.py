...
class ExperienceBalancedStoragePolicy(StoragePolicy):
    ...
    def subsample_single(self, data: AvalancheDataset, new_size: int):
        ...
        if removed_els > 0:
            ...
            ind6 = list(samples[targets == 6])
            ind5 = list(samples[targets == 5])
            ind1 = list(samples[targets == 1])
            ind2 = list(samples[targets == 2])
            samples = ind5 + ind1 + ind2
            np.random.shuffle(samples)
            samples = ind6 + samples
